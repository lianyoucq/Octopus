...
class DBConnectivity(GeneratedsSuper):
    ...
    def build(self, node):
        print("# the build function is invoked")
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
            nodeName_ = Tag_pattern_.match(child.tag).groups()[-1]
            self.buildChildren(child, node, nodeName_)
            print("# the child nodeName is {0}".format(nodeName_))
        print("ConnectionString is {0}".format(self.get_dbConnectString()))
        print("# the already_processed is {0}".format(already_processed))
        return self
